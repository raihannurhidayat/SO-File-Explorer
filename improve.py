...
class FileManager(QMainWindow):
    ...
    def init_ui(self):
        ...
        toolbar.addWidget(
            self.create_button(QStyle.SP_ArrowLeft, "Undo", self.perform_undo)
        )
        toolbar.addWidget(
            self.create_button(QStyle.SP_ArrowRight, "Redo", self.perform_redo)
        )
        toolbar.addWidget(
    ...
    def perform_delete(self):
        ...
        try:
            ...
            if os.path.isdir(path):
                ...
                os.remove(path)
            self.undo_stack.append(("restore", backup_path, path))
            self.redo_stack.clear()
    ...
    def create_new_file(self):
        ...
        if not ok:
            return
        base, ext = os.path.splitext(name or "File Baru.txt")
        file_name = self.get_available_name(base, ext)
    ...
    def perform_undo(self):
        ...
        self.handle_action(action, undo=True)
    ...
    def perform_redo(self):
        ...
        self.handle_action(action, undo=False)

    def handle_action(self, action, undo):
        try:
            if action[0] == "delete":
                if os.path.isdir(action[1]):
                    shutil.rmtree(action[1])
                elif os.path.isfile(action[1]):
                    os.remove(action[1])
                if undo:
                    self.redo_stack.append(("restore", action[1], action[1]))
            elif action[0] == "move":
                shutil.move(action[1], action[2])
                if undo:
                    self.redo_stack.append(("move", action[2], action[1]))
            elif action[0] == "rename":
                os.rename(action[1], action[2])
                if undo:
                    self.redo_stack.append(("rename", action[2], action[1]))
            elif action[0] == "restore":
                shutil.move(action[1], action[2])
                if undo:
                    self.redo_stack.append(("delete", action[2]))
            self.refresh_view()
            self.status.setText("Undo" if undo else "Redo")
        except Exception as e:
            self.show_error(str(e))
